...
const TASK_COLUMNS: Array<{ status: TaskStatus; title: string; columnClass: string }> = [
  {
    status: "TODO",
    title: "To Do",
    columnClass: "border-violet-200/70 bg-violet-50/40",
  },
  {
    status: "IN_PROGRESS",
    title: "In Progress",
    columnClass: "border-indigo-200/70 bg-indigo-50/40",
  },
  {
    status: "DONE",
    title: "Completed",
    columnClass: "border-fuchsia-200/70 bg-fuchsia-50/35",
  },
];
...
export function TaskList({
  ...
  const groupedTasks: Record<TaskStatus, Task[]> = {
    TODO: tasks.filter((task) => (task.status ?? "TODO") === "TODO"),
    ...
    <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {error && <p className="text-sm text-destructive">Error loading tasks: {error}</p>}
          ...
          {!loading && !error && selectedProposalKey && tasks.length > 0 && (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {TASK_COLUMNS.map((column) => (
                <div
                  key={column.status}
                  className={`rounded-xl border p-3 ${column.columnClass}`}
                >
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-foreground">{column.title}</p>
                    <Badge
                      variant={
                        getStatusVariant(column.status) as
                          | "status-todo"
                          | "status-progress"
                          | "status-done"
                      }
                      ...
                      return (
                        <div key={task.id} className="space-y-2">
                          {isEditing ? (
                            ...
                            <div className="rounded-lg border border-border/60 bg-card p-3 transition-all hover:border-primary/50 hover:shadow-sm">
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1 space-y-2">
                                  ...
                                  <div className="flex items-center gap-3 flex-wrap text-xs text-muted-foreground">
                                    <span className="font-mono text-primary/90">{truncateId(task.id)}</span>
